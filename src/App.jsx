...
const QuizPage = ({ type, data, title, colorClass, onBack }) => {
  ...
  const getMainFontSize = () => {
    if (mode === 'j-r') {
      return currentQuestion.type === 'combo' ? 'text-6xl sm:text-7xl' : 'text-8xl sm:text-9xl';
    } else {
      const textLen = currentQuestion.romaji.length;
      if (textLen > 6) return 'text-3xl sm:text-4xl';
      if (textLen > 2) return 'text-5xl sm:text-6xl';
      return 'text-6xl sm:text-7xl';
    }
  };

  const getButtonClass = (option) => {
    const baseClass = "relative w-full p-4 sm:p-5 rounded-xl text-xl font-bold transition-all duration-200 transform hover:-translate-y-1 shadow-md border-2 active:scale-95";
  ...
  return (
    <div className="flex items-center justify-center p-4 bn-font pt-10 pb-20 sm:pb-10">
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  <CategoryToggle label="টেনটেন" active={categories.dakuten} onClick={() => handleCategoryChange('dakuten')} theme={theme} />
          ...
          {!currentQuestion ? (
            ...
            <>
                ...
                <div className="relative group cursor-pointer" onClick={() => speak(currentQuestion.char)}>
                  <div className={`w-40 h-40 sm:w-48 sm:h-48 bg-white rounded-[2rem] shadow-xl border-4 ${theme.border} flex items-center justify-center relative overflow-hidden transition-transform active:scale-95`}>
                    {mode === 'j-r' ? (
                      ...
                      <span className={`${getMainFontSize()} font-bold ${theme.textDark} font-sans px-2 text-center select-none`}>{currentQuestion.romaji}</span>
                    )}
                  </div>
              ...
              <div className="h-16 flex items-center justify-center">
                {isAnswered ? (
                  <button onClick={generateQuestion} className={`flex items-center gap-2 ${theme.btn} text-white px-8 py-3 rounded-full font-bold shadow-lg hover:-translate-y-0.5 transition-all animate-in zoom-in duration-200 active:scale-95`}>
